...
cat << EOF > /etc/systcl.d/99-kubernetes-cri.conf 
...
curl -L http://download.opensuse.org/repositories/devel:/kubiuc:/libcontainers:/stable:/cri-o:/$VER/$OS/Release.key | apt-key add -
